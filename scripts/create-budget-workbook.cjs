...
const outputPath = path.resolve(process.cwd(), 'Budget_Aplikasi_ira_preregist.xlsx');
...
function header(row) {
  ...
  row.eachCell((cell) => {
    cell.border = { bottom: { style: 'thin', color: colors.border } };
  });
}
...
const calculations = scenarios.map((customers) => {
  ...
  const oneTime =
    assumptions.implementationHours * assumptions.implementationRate +
    assumptions.qaHours * assumptions.qaRate +
    assumptions.complianceBuffer;
  return {
    invitations,
    reminders,
    marketing,
    ...
    recurring,
    oneTime,
    firstYear: recurring * 12 + oneTime,
  };
});
...
title(
  as,
  'Asumsi Budget Aplikasi IRA Preregist',
  'Sel kuning dapat diubah. Tarif WhatsApp adalah asumsi sementara dan wajib disesuaikan dengan rate card Meta untuk market Indonesia dan kategori pesan yang disetujui.',
);
...
const assumptionRows = [
  ...
  [
    'Invitation messages per customer',
    assumptions.invitationPerCustomer,
    'message/customer',
    'Template verifikasi awal',
  ],
  [
    'Average reminders per customer',
    assumptions.remindersPerCustomer,
    'message/customer',
    'Rata-rata; maksimum aplikasi saat ini 3 per sesi',
  ],
  [
    'Marketing campaign messages per customer',
    assumptions.marketingPerCustomer,
    'message/customer',
    'Isi jika campaign diklasifikasikan Marketing',
  ],
  [
    'Estimated successful delivery rate',
    assumptions.deliveryRate,
    '%',
    'Meta menagih saat delivered; asumsi untuk budgeting',
  ],
  ...
  [
    ...
    'IDR/delivered message',
    'ASUMSI EDITABLE; cek rate card Meta',
  ],
  [
    'Meta/BSP fixed monthly fee',
    assumptions.metaFixedFee,
    'IDR/month',
    '0 untuk direct Cloud API; partner fee bila ada diisi di sini',
  ],
  [
    'Forward geocode requests per customer',
    assumptions.forwardPerCustomer,
    'request/customer',
    'Alamat baru/ubah alamat',
  ],
  [
    'Reverse geocode requests per customer',
    assumptions.reversePerCustomer,
    'request/customer',
    'GPS terbaik per submit pada implementasi saat ini',
  ],
  ['Google Geocoding free usage cap', assumptions.geocodeFreeCap, 'request/month/SKU', 'Baseline global Essentials'],
  [
    'Google Geocoding price',
    assumptions.geocodeUsdPer1000,
    'USD/1,000 requests',
    'Baseline global, tier pertama setelah free cap',
  ],
  [
    'Google Maps JS map loads per customer',
    assumptions.mapLoadsPerCustomer,
    'load/customer',
    '0 berarti tetap memakai Leaflet/OSM seperti aplikasi saat ini',
  ],
  ['Google Dynamic Maps free usage cap', assumptions.mapFreeCap, 'load/month/SKU', 'Baseline global Essentials'],
  [
    'Google Dynamic Maps price',
    assumptions.mapUsdPer1000,
    'USD/1,000 loads',
    'Baseline global, tier pertama setelah free cap',
  ],
  ...
  [
    'Compliance/privacy/setup buffer',
    assumptions.complianceBuffer,
    'IDR one-time',
    'Legal, consent flow, template/setup contingency',
  ],
];
...
title(
  wa,
  'WhatsApp Cloud API Calculator',
  'Perhitungan menggunakan delivered message. Rate Utility/Marketing diambil dari sel Asumsi dan sengaja dibuat editable karena rate Meta berubah berdasarkan market, kategori, dan volume.',
);
...
waRows.forEach((value, index) => {
  wa.getRow(5 + index).getCell(1).value = value[0];
  bodyStyle(wa.getRow(5 + index), index % 2 ? colors.white : colors.lightBlue);
});
...
for (let i = 0; i < 3; i += 1) {
  ...
  const formulas = [
    ...
    `=${col}14*${col}17`,
    '=Asumsi!$B$14',
    `=SUM(${col}16,${col}18,${col}19)`,
  ];
  formulas.forEach((f, index) =>
    formula(
      wa.getCell(5 + index, 2 + i),
      f,
      waResults[index](i),
      index === 4
        ? percentFormat
...
wa.getRow(20).font = { bold: true };
wa.getRow(20).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.green } };
wa.getColumn(1).width = 38;
wa.getColumn(2).width = 18;
wa.getColumn(3).width = 18;
wa.getColumn(4).width = 18;
wa.freezePanes = 'B5';

const maps = workbook.addWorksheet('Maps Calculator');
title(
  maps,
  'Google Maps / Geocoding Calculator',
  'Aplikasi saat ini memakai Leaflet + OpenStreetMap untuk peta dan hanya membuat deep link Google Maps. Isi map loads/customer jika ingin migrasi ke Maps JavaScript API.',
);
addScenarioHeaders(maps, 4);
const mapRows = [
  ['Customer per month'],
  ['Forward geocode requests/customer'],
  ['Reverse geocode requests/customer'],
  ['Google Maps JS map loads/customer'],
  ['Forward geocode requests'],
  ['Reverse geocode requests'],
  ['Total Geocoding requests'],
  ['Free usage cap'],
  ['Billable Geocoding requests'],
  ['Geocoding price USD/1,000'],
  ['Geocoding cost USD'],
  ['Geocoding cost IDR'],
  ['Google Maps JS map loads'],
  ['Map free usage cap'],
  ['Billable map loads'],
  ['Dynamic Maps price USD/1,000'],
  ['Dynamic Maps cost USD'],
  ['Dynamic Maps cost IDR'],
  ['Total Google monthly cost'],
];
mapRows.forEach((value, index) => {
  maps.getRow(5 + index).getCell(1).value = value[0];
  bodyStyle(maps.getRow(5 + index), index % 2 ? colors.white : colors.lightBlue);
});
const mapResults = [
  (i) => scenarios[i],
  () => assumptions.forwardPerCustomer,
  () => assumptions.reversePerCustomer,
  () => assumptions.mapLoadsPerCustomer,
  (i) => scenarios[i] * assumptions.forwardPerCustomer,
  (i) => scenarios[i] * assumptions.reversePerCustomer,
  (i) => calculations[i].geoRequests,
  () => assumptions.geocodeFreeCap,
  (i) => calculations[i].geoBillable,
  () => assumptions.geocodeUsdPer1000,
  (i) => (calculations[i].geoBillable / 1000) * assumptions.geocodeUsdPer1000,
  (i) => calculations[i].geoCost,
  (i) => calculations[i].mapLoads,
  () => assumptions.mapFreeCap,
  (i) => calculations[i].mapBillable,
  () => assumptions.mapUsdPer1000,
  (i) => (calculations[i].mapBillable / 1000) * assumptions.mapUsdPer1000,
  (i) => calculations[i].mapCost,
  (i) => calculations[i].geoCost + calculations[i].mapCost,
];
for (let i = 0; i < 3; i += 1) {
  const col = String.fromCharCode(66 + i);
  const formulas = [
...
maps.getRow(24).font = { bold: true };
maps.getRow(24).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.green } };
maps.getColumn(1).width = 38;
maps.getColumn(2).width = 18;
maps.getColumn(3).width = 18;
maps.getColumn(4).width = 18;
maps.freezePanes = 'B5';

const infra = workbook.addWorksheet('Infra & Setup');
title(
  infra,
  'Infrastructure dan One-time Setup',
  'Angka hosting, database, Redis, domain, tenaga implementasi, dan compliance adalah estimasi budgeting yang dapat diganti sesuai vendor/penawaran aktual.',
);
addScenarioHeaders(infra, 4);
const infraRows = [
  ['Application/VPS server'],
  ['Managed PostgreSQL/PostGIS'],
  ['Redis/BullMQ'],
  ['Backup/object storage'],
  ['Monitoring/email/alerting'],
...
infraRows.forEach((value, index) => {
  infra.getRow(5 + index).getCell(1).value = value[0];
  bodyStyle(infra.getRow(5 + index), index % 2 ? colors.white : colors.lightBlue);
});
for (let i = 0; i < 3; i += 1) {
  const col = String.fromCharCode(66 + i);
  const monthly = [
    ...
    '=Asumsi!$B$14',
    `=SUM(${col}5:${col}11)`,
    '=Asumsi!$B$30*Asumsi!$B$31',
    '=Asumsi!$B$32*Asumsi!$B$33',
    '=Asumsi!$B$34',
    `=SUM(${col}13:${col}15)`,
  ];
  const values = [
...
infra.getRow(12).font = { bold: true };
infra.getRow(12).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.green } };
infra.getRow(16).font = { bold: true };
infra.getRow(16).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.orange } };
infra.getColumn(1).width = 42;
infra.getColumn(2).width = 18;
infra.getColumn(3).width = 18;
infra.getColumn(4).width = 18;
infra.freezePanes = 'B5';

const summary = workbook.addWorksheet('Ringkasan', { views: [{ showGridLines: false }] });
title(
  summary,
  'Budget Aplikasi IRA Preregist',
  'Estimasi awal per 1 September 2026. Gunakan sheet Asumsi untuk mengganti volume dan tarif. Angka ini bukan quotation vendor.',
);
addScenarioHeaders(summary, 4);
const summaryLabels = [
  'WhatsApp monthly',
  'Google Maps/Geocoding monthly',
  ...
  'First-year monthly equivalent',
];
summaryLabels.forEach((label, index) => {
  summary.getRow(5 + index).getCell(1).value = label;
  bodyStyle(summary.getRow(5 + index), index % 2 ? colors.white : colors.lightBlue);
});
for (let i = 0; i < 3; i += 1) {
  const col = String.fromCharCode(66 + i);
  const calc = calculations[i];
  const formulas = [
    `='WA Calculator'!${col}20`,
    `='Maps Calculator'!${col}23`,
    `='Infra & Setup'!${col}12`,
    `=SUM(${col}5:${col}7)`,
    `=${col}8*Asumsi!$B$29`,
    `=SUM(${col}8:${col}9)`,
  ...
  const values = [
    calc.waUtility + calc.waMarketing,
    calc.geoCost + calc.mapCost,
    calc.infra,
    calc.subtotal,
    calc.contingency,
    calc.recurring,
...
summary.getRow(10).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.green } };
summary.getRow(13).font = { bold: true };
summary.getRow(13).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors.orange } };
summary.getRow(16).values = ['Catatan penting'];
section(summary.getCell('A16'));
...
notes.forEach((note, index) => {
  summary.mergeCells(`A${17 + index}:D${17 + index}`);
  summary.getCell(`A${17 + index}`).value = note;
  summary.getCell(`A${17 + index}`).alignment = { wrapText: true, vertical: 'top' };
  summary.getRow(17 + index).height = 28;
});
...
checklist.getRow(4).values = ['Area', 'Item', 'Status', 'Catatan'];
header(checklist.getRow(4));
const checklistRows = [
  ...
  [
    'Google',
    'Pisahkan server key dan browser key',
    'Pending',
    'IP restriction untuk server; referrer restriction untuk browser',
  ],
  [
    'Application',
    'Google adapter memetakan response ke GeocodingResult',
    'Pending',
    'Adapter saat ini masih generic /forward dan /reverse',
  ],
  [
    'Application',
    'Meta adapter membaca messages[0].id',
    'Pending',
    'Response Cloud API bukan providerMessageId top-level',
  ],
...
checklistRows.forEach((values, index) => {
  const row = checklist.getRow(5 + index);
  row.values = values;
  bodyStyle(row, index % 2 ? colors.white : colors.lightBlue);
  row.getCell(3).dataValidation = { type: 'list', allowBlank: false, formulae: ['"Pending,In Progress,Done,Blocked"'] };
});
checklist.getColumn(1).width = 18;
checklist.getColumn(2).width = 55;
checklist.getColumn(3).width = 18;
checklist.getColumn(4).width = 65;
checklist.freezePanes = 'A5';
checklist.autoFilter = { from: 'A4', to: `D${4 + checklistRows.length}` };

const sources = workbook.addWorksheet('Sumber');
title(
  sources,
  'Sumber Harga dan Referensi',
  'URL resmi/primer yang digunakan sebagai referensi workbook. Tarif dinamis tetap harus dicek kembali saat pembelian atau production launch.',
);
sources.getRow(4).values = ['Provider', 'Referensi', 'URL', 'Tanggal cek', 'Keterangan'];
header(sources.getRow(4));
const sourceRows = [
  [
    'Meta',
    ...
    '2026-09-01',
    'Per delivered message; market + category',
  ],
  [
    'Meta',
    'Business Messaging Policy Indonesia',
    'https://business.whatsapp.com/policy/preview?lang=id_ID',
    '2026-09-01',
    'Consent, approved templates, opt-out',
  ],
  [
    'Meta',
    ...
    '2026-09-01',
    'Endpoint and required token/Phone Number ID',
  ],
  [
    'Meta',
    'Templates API collection',
  ...
  [
    ...
    '2026-09-01',
    'Geocoding: 10k free; baseline USD 5/1k; Dynamic Maps: USD 7/1k baseline',
  ],
  [
    'Google',
    ...
    '2026-09-01',
    'Key restrictions and server/browser separation',
  ],
  [
    'Google',
    ...
    '2026-09-01',
    'Optional interactive Google map',
  ],
  [
    'Local',
    ...
    'README.md',
    '2026-09-01',
    'NestJS, PostgreSQL/PostGIS, Redis/BullMQ, Leaflet/OSM',
  ],
];
sourceRows.forEach((values, index) => {
  const row = sources.getRow(5 + index);
  row.values = values;
  bodyStyle(row, index % 2 ? colors.white : colors.lightBlue);
  row.getCell(3).font = { color: '0563C1', underline: true };
});
sources.getColumn(1).width = 14;
sources.getColumn(2).width = 34;
sources.getColumn(3).width = 75;
sources.getColumn(4).width = 16;
sources.getColumn(5).width = 58;
sources.freezePanes = 'A5';
sources.autoFilter = { from: 'A4', to: `E${4 + sourceRows.length}` };

for (const sheet of workbook.worksheets) {
  sheet.eachRow((row) => {
    row.alignment = { ...row.alignment, vertical: row.alignment?.vertical || 'top' };
  });
