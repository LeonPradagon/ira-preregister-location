...
function styleTitle(sheet, main, sub) {
  ...
  sheet.getCell('A1').value = main;
  sheet.getCell('A1').font = { bold: true, size: 16, color: C.white };
...
function header(row) {
  row.font = { bold: true, color: C.white };
  row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.navy } };
  row.alignment = { wrapText: true, vertical: 'middle' };
}
function body(row, fill) {
  row.eachCell((cell) => {
    cell.alignment = { wrapText: true, vertical: 'top' };
    cell.border = { bottom: { style: 'hair', color: C.border } };
    if (fill) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fill } };
  });
}
function setFormula(cell, f, result, format) {
...
styleTitle(
  input,
  'Input Budget: 1 Juta User',
  'Sel kuning dapat diubah. Tarif WhatsApp diambil dari rate card Indonesia Meta efektif 1 Juli 2026. Semua angka harga di workbook menggunakan IDR.',
);
input.getRow(4).values = ['Parameter', 'Nilai', 'Satuan', 'Catatan'];
header(input.getRow(4));
const inputs = [
  ...
  [
    'Tarif WhatsApp Utility tier awal',
    a.utilityRate,
    'IDR/message',
    'Rate card Indonesia; biaya utility dihitung bertingkat di sheet WA Rate Card',
  ],
  [
    'Tarif WhatsApp Marketing',
    a.marketingRate,
    'IDR/message',
    'Rate card Indonesia; marketing tidak memakai volume tier pada kartu ini',
  ],
  ...
  [
    'Google Maps JS loads per user',
    a.mapLoads,
    'load/user',
    '0 berarti tetap Leaflet/OSM; isi jika migrasi ke Google Maps JS',
  ],
...
inputs.forEach((values, i) => {
  const row = input.getRow(5 + i);
  row.values = values;
  body(row, i % 2 ? C.white : C.light);
  row.getCell(2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.yellow } };
  const unit = values[2];
  if (unit === '%') row.getCell(2).numFmt = percent;
  if (unit.startsWith('IDR')) row.getCell(2).numFmt = idrDecimal;
});
input.getColumn(1).width = 38;
input.getColumn(2).width = 18;
input.getColumn(3).width = 25;
input.getColumn(4).width = 78;
input.freezePanes = 'A5';
input.autoFilter = { from: 'A4', to: `D${4 + inputs.length}` };

const rateCard = workbook.addWorksheet('WA Rate Card');
styleTitle(
  rateCard,
  'Meta WhatsApp Rate Card - Indonesia',
  'Tarif dari PDF rate card yang Anda kirim, efektif 1 Juli 2026. Volume tier berlaku per kategori per bulan; utility pada estimasi dihitung progresif per tier.',
);
rateCard.getRow(4).values = [
  'Kategori',
  'Mulai pesan/bulan',
  'Sampai pesan/bulan',
  'Tarif IDR/message',
...
header(rateCard.getRow(4));
utilityTiers.forEach((tier, i) => {
  const row = rateCard.getRow(5 + i);
  row.values = [
    'Utility',
    tier.from,
    tier.to,
    ...
    null,
    null,
    i === 0 ? 'List rate' : `Tier rate (${i * 5}% discount)`,
  ];
  body(row, i % 2 ? C.white : C.light);
  setFormula(
    row.getCell(5),
    `=MAX(0,MIN(WhatsApp!$B$9,IF(C${5 + i}="",1E+99,C${5 + i}))-B${5 + i}+1)`,
    utilityTierUsage[i],
    number,
  );
  setFormula(row.getCell(6), `=D${5 + i}*E${5 + i}`, utilityTierUsage[i] * tier.rate, idrDecimal);
  row.getCell(3).numFmt = number;
  row.getCell(4).numFmt = idrDecimal;
});
rateCard.getRow(11).values = ['Total Utility', null, null, null, null, null, 'Dipakai oleh sheet WhatsApp'];
body(rateCard.getRow(11), C.green);
setFormula(rateCard.getCell('F11'), '=SUM(F5:F10)', waUtility, idrDecimal);
rateCard.getRow(11).font = { bold: true };
rateCard.getRow(13).values = ['Kategori', 'Rate awal IDR/message', 'Rate tertinggi IDR/message', 'Catatan'];
header(rateCard.getRow(13));
[
  ...
  ['Authentication-International', 1940.13, 1455.1, 'Volume tier; tidak dipakai dalam baseline ini'],
  ['Service', null, null, 'Tidak dikenakan biaya pada rate card yang dirujuk'],
].forEach((values, i) => {
  const row = rateCard.getRow(14 + i);
  row.values = values;
  body(row, i % 2 ? C.white : C.light);
  row.getCell(2).numFmt = idrDecimal;
  row.getCell(3).numFmt = idrDecimal;
});
rateCard.getColumn(1).width = 30;
rateCard.getColumn(2).width = 20;
rateCard.getColumn(3).width = 20;
rateCard.getColumn(4).width = 22;
rateCard.getColumn(5).width = 20;
rateCard.getColumn(6).width = 22;
rateCard.getColumn(7).width = 48;
rateCard.freezePanes = 'A5';

const wa = workbook.addWorksheet('WhatsApp');
styleTitle(
  wa,
  'WhatsApp Cloud API Cost',
  'Perhitungan API-only untuk market Indonesia. Utility memakai volume tier dari sheet WA Rate Card; biaya dihitung berdasarkan pesan delivered.',
);
wa.getRow(4).values = ['Komponen', 'Nilai', 'Satuan', 'Formula/penjelasan'];
header(wa.getRow(4));
const waRows = [
...
waRows.forEach((values, i) => {
  const row = wa.getRow(5 + i);
  row.values = [values[0], null, i <= 4 ? 'message' : 'IDR', values[3]];
  body(row, i % 2 ? C.white : C.light);
  setFormula(row.getCell(2), values[1], values[2], i <= 4 ? number : idrDecimal);
});
wa.getRow(14).font = { bold: true };
wa.getRow(14).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.green } };
wa.getColumn(1).width = 34;
wa.getColumn(2).width = 22;
wa.getColumn(3).width = 22;
wa.getColumn(4).width = 52;
wa.freezePanes = 'A5';

const google = workbook.addWorksheet('Google Maps');
styleTitle(
  google,
  'Google Maps / Geocoding Cost',
  'Semua biaya ditampilkan dalam IDR. Baseline memakai 1 Google Maps JS map load per user; ubah menjadi 0 jika hanya memakai geocoding atau deep link.',
);
google.getRow(4).values = ['Komponen', 'Nilai', 'Satuan', 'Formula/penjelasan'];
header(google.getRow(4));
const googleRows = [
...
googleRows.forEach((values, i) => {
  const row = google.getRow(5 + i);
  row.values = [
    values[0],
    null,
    i === 5 || i === 10 ? 'IDR/1.000' : i === 6 || i === 11 || i === 12 ? 'IDR' : 'request/load',
    values[3],
  ];
  body(row, i % 2 ? C.white : C.light);
  const format = i === 5 || i === 10 ? idrDecimal : i === 6 || i === 11 || i === 12 ? idrDecimal : number;
  setFormula(row.getCell(2), values[1], values[2], format);
});
google.getRow(17).font = { bold: true };
google.getRow(17).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.green } };
google.getColumn(1).width = 38;
google.getColumn(2).width = 22;
google.getColumn(3).width = 22;
google.getColumn(4).width = 52;
google.freezePanes = 'A5';

const summary = workbook.addWorksheet('Ringkasan', { views: [{ showGridLines: false }] });
styleTitle(
  summary,
  'Ringkasan Budget API: 1 Juta User',
  'Budget ini hanya mencakup WhatsApp Cloud API dan Google Maps/Geocoding API. Baseline memakai 1 Google Maps JS load per user. Hosting, database, Redis, domain, development, dan pajak belum termasuk.',
);
summary.getRow(4).values = ['Komponen', 'Per 1 siklus', 'Siklus/tahun', 'Per tahun'];
header(summary.getRow(4));
const summaryRows = [
...
summaryRows.forEach((values, i) => {
  ...
  row.values = [values[0], null, null, null];
  body(row, i % 2 ? C.white : C.light);
  setFormula(
    row.getCell(2),
    i === 0
  ...
  setFormula(row.getCell(3), '=Input!$B$20', a.cycles, number);
  setFormula(
    row.getCell(4),
    i === 5 ? '=B9*C9' : `=B${5 + i}*C${5 + i}`,
    i === 5 ? cycleTotal * a.cycles : values[1] * a.cycles,
    idrDecimal,
  );
});
summary.getRow(9).font = { bold: true };
summary.getRow(9).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.green } };
summary.getRow(10).font = { bold: true };
summary.getRow(10).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.orange } };
summary.mergeCells('A13:D13');
summary.getCell('A13').value = 'Catatan';
summary.getCell('A13').font = { bold: true, color: C.navy };
summary.getCell('A13').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: C.blue } };
const notes = [
...
notes.forEach((note, i) => {
  summary.mergeCells(`A${14 + i}:D${14 + i}`);
  summary.getCell(`A${14 + i}`).value = note;
  summary.getCell(`A${14 + i}`).alignment = { wrapText: true, vertical: 'top' };
  summary.getRow(14 + i).height = 34;
});
summary.getColumn(1).width = 40;
summary.getColumn(2).width = 22;
summary.getColumn(3).width = 18;
summary.getColumn(4).width = 22;
summary.freezePanes = 'A5';
...
[
  [
    'Meta',
    'WhatsApp Rate Card Indonesia - PDF user',
    'https://developers.facebook.com/docs/whatsapp/pricing/volume-tiers',
    '2026-09-01',
    'Tarif IDR; efektif 1 Juli 2026; utility/authentication volume tiers',
  ],
  [
    'Meta',
  ...
  [
    ...
    '2026-09-01',
    'Geocoding dan Dynamic Maps baseline',
  ],
  [
    'Google',
  ...
  [
    ...
    '2026-09-01',
    'Restrict key dan pisahkan server/browser key',
  ],
].forEach((values, i) => {
  const row = sources.getRow(5 + i);
  row.values = values;
  body(row, i % 2 ? C.white : C.light);
  row.getCell(3).font = { color: '0563C1', underline: true };
});
sources.getColumn(1).width = 14;
sources.getColumn(2).width = 42;
sources.getColumn(3).width = 75;
sources.getColumn(4).width = 16;
sources.getColumn(5).width = 62;
sources.freezePanes = 'A5';
sources.autoFilter = { from: 'A4', to: 'E11' };
