...
export interface RuntimeValidationConfig extends Record<string, unknown> {
  ...
  ACTIVE_SESSION_TTL_DAYS: number;
...
export class ValidationConfigService {
  private fromEnvironment(): RuntimeValidationConfig {
    return {
      ...
      ACTIVE_SESSION_TTL_DAYS: Number(process.env.ACTIVE_SESSION_TTL_DAYS ?? 7),
      REMINDER_LINK_TTL_HOURS: Number(process.env.REMINDER_LINK_TTL_HOURS ?? 24),
      UNOPENED_LINK_REMINDER_DELAY_DAYS: Number(process.env.UNOPENED_LINK_REMINDER_DELAY_DAYS ?? 1),
      UNOPENED_LINK_REMINDER_INTERVAL_DAYS: Number(process.env.UNOPENED_LINK_REMINDER_INTERVAL_DAYS ?? 1),
