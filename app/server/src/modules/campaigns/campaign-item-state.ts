...
export class CampaignItemState {
  static async claim(itemId: string) {
    const [item] = await db.update(verificationCampaignItems)
  ...
  static async markSent(itemId: string, providerMessageId: string, sentAt: Date) {
    return db.transaction(async (tx) => {
      const [item] = await tx.update(verificationCampaignItems)
        .set({ status: 'SENT', sentAt, providerMessageId, processingStartedAt: null, lastError: null, updatedAt: sentAt })
      ...
      if (!item) return null;
      await tx.update(verificationCampaigns).set({ sentCount: sql`${verificationCampaigns.sentCount} + 1`, updatedAt: sentAt }).where(eq(verificationCampaigns.id, item.campaignId));
      return item;
    });
  }
  ...
  static async markFailure(itemId: string, retryCount: number, terminal: boolean, error: string, failedAt: Date) {
    return db.transaction(async (tx) => {
      ...
      await tx.update(verificationCampaigns).set({ failedCount: sql`${verificationCampaigns.failedCount} + 1`, updatedAt: failedAt }).where(eq(verificationCampaigns.id, item.campaignId));
      return item;
    });
  }
  ...
  static async markOptedOut(itemId: string, changedAt: Date) {
    return db.transaction(async (tx) => {
      const [item] = await tx.update(verificationCampaignItems)
        .set({ status: 'OPTED_OUT', processingStartedAt: null, lastError: 'CUSTOMER_OPTED_OUT', failedAt: changedAt, updatedAt: changedAt })
        .where(and(eq(verificationCampaignItems.id, itemId), inArray(verificationCampaignItems.status, ['PENDING', 'PROCESSING'])))
        .returning({ id: verificationCampaignItems.id, campaignId: verificationCampaignItems.campaignId });
      if (!item) return null;
      await tx.update(verificationCampaigns).set({ failedCount: sql`${verificationCampaigns.failedCount} + 1`, optedOutCount: sql`${verificationCampaigns.optedOutCount} + 1`, updatedAt: changedAt }).where(eq(verificationCampaigns.id, item.campaignId));
      return item;
    });
  }

  static async applyDeliveryStatus(providerMessageId: string, status: CampaignDeliveryStatus, occurredAt: Date) {
    const previousStatuses = status === 'DELIVERED'
      ? ['SENT']
      : status === 'READ'
        ? ['SENT', 'DELIVERED']
        : status === 'FAILED'
    ...
    return db.transaction(async (tx) => {
      const [item] = await tx.update(verificationCampaignItems)
        .set(status === 'DELIVERED'
          ...
          : { status: 'FAILED', processingStartedAt: null, failedAt: occurredAt, lastError: 'PROVIDER_REPORTED_FAILURE', updatedAt: occurredAt })
        .where(and(eq(verificationCampaignItems.providerMessageId, providerMessageId), inArray(verificationCampaignItems.status, previousStatuses)))
        .returning({ id: verificationCampaignItems.id, campaignId: verificationCampaignItems.campaignId });
      if (!item) return null;
      if (status === 'FAILED') await tx.update(verificationCampaigns).set({ failedCount: sql`${verificationCampaigns.failedCount} + 1`, updatedAt: occurredAt }).where(eq(verificationCampaigns.id, item.campaignId));
      return item;
    });
  }
  ...
  static async reconcile(campaignId: string) {
    ...
    if (!campaign) return;
    const nextStatus = campaign.status === 'PAUSED' ? 'PAUSED' : campaign.materializationComplete && Number(counts.pending) === 0 ? 'COMPLETED' : 'RUNNING';
    await db.update(verificationCampaigns).set({ sentCount: Number(counts.sent), failedCount: Number(counts.failed), optedOutCount: Number(counts.optedOut), status: nextStatus, updatedAt: new Date() }).where(eq(verificationCampaigns.id, campaignId));
  }
}
