 ...
 */
const normalizeAdministrativeArea = (value: string): string =>
...
export function decideValidation(
  ...
  const addressTextMismatch =
    !addressIncomplete &&
    (!administrativeLevelsMatch || !streetIsAcceptablySimilar || houseNumberMatch === false || addressScore < 0.6);
  const distanceFromReferenceMeters = hasReferenceLocation
    ? distanceMeters(bestSample, {
  ...
  const outsideHomeRadius =
    precisionOk && distanceFromReferenceMeters != null && distanceFromReferenceMeters > config.homeRadiusMeters;
