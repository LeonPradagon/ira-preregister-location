...
export function applyApprovalPolicy(
  input: AutoApprovalPolicyInput & { reasonCodes: string[] },
  ...
  if (engineResultCanAutoApprove && shouldAutoApprove({ ...input, result: 'LOCATION_VALID' })) {
    return {
      result: 'LOCATION_VALID',
      reasonCodes: [
        ...input.reasonCodes.filter(
          (reasonCode) => !['AUTOMATED_VALIDATION_PASSED', 'MANUAL_REVIEW_REQUIRED'].includes(reasonCode),
        ),
        'AUTO_APPROVED',
      ],
      autoApproved: true,
  ...
  return {
    ...
    reasonCodes:
      input.result === 'LOCATION_VALID'
        ? [
            ...input.reasonCodes.filter((reasonCode) => reasonCode !== 'LOCATION_VALID'),
