...
export class CustomerImportService implements OnModuleDestroy {
  ...
  private readonly queue = new Queue(queueNames.imports, { connection: this.connection });
  ...
  async importLegacy(admin: RequestAdmin, file: UploadedCustomerFile) {
    ...
    const sourcePath =
      file.path ||
      join(
        (temporaryDirectory = await mkdtemp(join(tmpdir(), 'ira_preregist-customer-import-'))),
        `customers${extension}`,
      );
    try {
      ...
      const { stdout } = await execFileAsync(
        process.execPath,
        [resolve(import.meta.dirname, '../../../scripts/import-prereg-xlsx.mjs'), sourcePath],
        { cwd: resolve(import.meta.dirname, '../../..'), env: process.env, maxBuffer: 4 * 1024 * 1024 },
      );
      ...
      throw new BadRequestException(
        (childError.stderr?.trim() || childError.message || 'File gagal diproses.').replace(/\s+/g, ' ').slice(0, 500),
      );
    } finally {
  ...
  async import(admin: RequestAdmin, file: UploadedCustomerFile) {
    ...
    if (file.size > MAX_UPLOAD_SIZE_BYTES)
      throw new BadRequestException('Ukuran file maksimal 50 MB. Gunakan beberapa file batch jika data lebih besar.');
    ...
    try {
      // The upload directory and persistent Docker volume may use different
      // filesystems. Copy first; the temporary upload is removed in finally.
      if (file.path) await copyFile(file.path, storedPath);
      else await writeFile(storedPath, file.buffer!);
      await db.insert(importJobs).values({
        id: jobId,
        fileName: file.originalname,
        // Persist only the filename. The API can run on Windows while the
        // worker runs in Linux Docker, so an absolute host path is not portable.
  ...
  async get(admin: RequestAdmin, id: string) {
    const [job] = await db
      .select()
      .from(importJobs)
      .where(and(eq(importJobs.id, id), eq(importJobs.createdBy, admin.id)))
      .limit(1);
