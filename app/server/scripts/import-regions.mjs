...
const insertChunks = async (client, table, columns, rows, valueFactory) => {
  for (let offset = 0; offset < rows.length; offset += 500) {
    ...
    await client.query(
      `INSERT INTO ${table} (${columns.join(', ')}) VALUES ${values.join(', ')} ON CONFLICT (${columns[0]}) DO UPDATE SET ${columns
        .slice(1)
...
const main = async () => {
  ...
  for (const match of regionsSql.matchAll(regionPattern)) {
    ...
    regionRows.push({
      ...
      parentCode: separator === -1 ? null : code.slice(0, separator),
      level: code.split('.').length,
    });
  }
  const postalMap = JSON.parse(postalJson);
  const postalRows = Object.entries(postalMap).map(([regionCode, postalCode]) => ({
    regionCode,
  ...
  try {
    ...
    await client.query(
      `CREATE TABLE IF NOT EXISTS administrative_regions (code varchar(13) PRIMARY KEY NOT NULL, name varchar(100) NOT NULL, parent_code varchar(13), level integer NOT NULL)`,
    );
    await client.query(
    ...
    await client.query(
      `CREATE TABLE IF NOT EXISTS region_postal_codes (region_code varchar(13) PRIMARY KEY NOT NULL, postal_code varchar(5))`,
    );
    ...
    await insertChunks(
      ...
      regionRows,
      (row) => [row.code, row.name, row.parentCode, row.level],
    );
    await insertChunks(client, 'region_postal_codes', ['region_code', 'postal_code'], postalRows, (row) => [
      row.regionCode,
      row.postalCode,
    ]);
