...
import { shouldShowCustomerConfirmation, shouldShowLocationRetry, shouldShowReminderResume } from '../../src/lib/customerVerificationFlow';

describe('customer verification confirmation flow', () => {
  it.each(['CREATED', 'MESSAGE_SENT', 'LINK_OPENED', 'WAITING_FOR_HOME', 'REMINDER_LIMIT_REACHED'])('shows confirmation for an unconfirmed %s session', (status) => {
  ...
  it('shows a retry action when the same link is reopened during an interrupted GPS capture', () => {
    expect(shouldShowLocationRetry('GPS_CAPTURING', 'CONFIRMED', false)).toBe(true);
  });

  it('keeps location verification available after reminders have been scheduled', () => {
    expect(shouldShowLocationRetry('REMINDER_LIMIT_REACHED', 'CONFIRMED', false)).toBe(true);
  });
