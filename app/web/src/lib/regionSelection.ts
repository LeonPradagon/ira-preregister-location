...
const canonicalRegionName = (value: string) =>
  value
    .trim()
...
export const regionOptionValue = (option: RegionOption) =>
  /^daerah khusus ibukota jakarta$/i.test(option.name.trim()) ? 'DKI Jakarta' : option.name.trim();
