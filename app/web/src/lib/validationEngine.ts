...
export function evaluateBestGpsSample(samples: GpsSample[]): { bestSample: GpsSample; averageAccuracy: number; isConsistent: boolean; sampleSpreadMeters: number } {
  ...
  return { bestSample, averageAccuracy, isConsistent: maximumSpread <= 100, sampleSpreadMeters: Math.round(maximumSpread * 10) / 10 };
}
...
export function formatAddressForDisplay(rawAddress?: string): string {
  return (rawAddress ?? '').split(',').map((part) => part.replace(plusCodePattern, ' ').replace(/\s+/g, ' ').trim()).filter(Boolean).join(', ');
}

export function isIncompleteAddress(address: { rawAddress?: string; postalCode?: string; street?: string; houseNumber?: string }): boolean {
  const values = [address.postalCode, address.street, address.houseNumber];
  const hasPlaceholder = values.some((value) => !value?.trim() || ['unknown', 'tidak diketahui', 'tanpa nomor', 'n/a', 'na', '-', '00000'].includes(value.trim().toLowerCase()));
  const streetIsOnlyPlusCode = Boolean(address.street?.trim().match(/^[23456789cfghjmpqrvwx]{4,8}\+(?:[23456789cfghjmpqrvwx]{3}\d|[23456789cfghjmpqrvwx]{4})$/i));
