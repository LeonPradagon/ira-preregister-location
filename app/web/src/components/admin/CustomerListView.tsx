...
const CUSTOMER_CHECK_STATUS_LABEL: Record<string, string> = {
  ...
  MESSAGE_SENT: 'customers.checkStatus.MESSAGE_SENT',
  LINK_OPENED: 'customers.checkStatus.LINK_OPENED',
  CONSENTED: 'customers.checkStatus.CONSENTED',
  GPS_CAPTURING: 'customers.checkStatus.GPS_CAPTURING',
  LOCATION_VALID: 'customers.checkStatus.LOCATION_VALID',
  LOW_GPS_ACCURACY: 'customers.checkStatus.LOW_GPS_ACCURACY',
  LOCATION_MISMATCH: 'customers.checkStatus.LOCATION_MISMATCH',
  CUSTOMER_DATA_MISMATCH: 'customers.checkStatus.CUSTOMER_DATA_MISMATCH',
  MANUAL_REVIEW: 'customers.checkStatus.MANUAL_REVIEW',
  WAITING_FOR_HOME: 'customers.checkStatus.WAITING_FOR_HOME',
  ADDRESS_PROPOSED: 'customers.checkStatus.ADDRESS_PROPOSED',
};
...
export const CustomerListView: React.FC<CustomerListViewProps> = ({ onSelectCustomer }) => {
  const {
    customers,
    customerPage,
    dashboardSummary,
  ...
  const resetCustomerForm = () => {
    ...
    setNewCustName('');
    setNewCustPhone('+62');
    setNewCustExtId(generateCustomerExternalId());
    setNewCustStreet('');
    setNewCustHouseNo('');
    setNewCustAddressDetail('');
    setNewCustDistrict('');
    setNewCustSubdistrict('');
    setNewCustCity('');
    setNewCustProvince('');
    setNewCustPostalCode('');
    setNewCustLat('');
    setNewCustLng('');
    setNewCustStatus('PENDING_INSTALLATION');
    setFormError('');
    setRegionCodes({});
    setRegionOptions({ province: [], city: [], district: [], subdistrict: [] });
    setRegionError('');
  };
  ...
  useEffect(() => {
    ...
    const loadRegions = async () => {
      ...
      if (!city) {
        setRegionOptions((current) => ({ ...current, city: cities }));
        setRegionCodes({ province: province.code });
        return;
      }
      ...
      if (!district) {
        setRegionOptions((current) => ({ ...current, city: cities, district: districts }));
        setRegionCodes({ province: province.code, city: city.code });
        return;
      }
      ...
      setRegionCodes({
        ...
        ...(subdistrict ? { subdistrict: subdistrict.code } : {}),
      });
    };
    void loadRegions().catch((error: unknown) => {
      if (active) setRegionError(error instanceof Error ? error.message : 'Data wilayah gagal dimuat.');
    });
    return () => {
      active = false;
    };
  ...
  const handleRegionChange = async (field: RegionLevel, value: string) => {
    ...
    try {
      ...
      if (requestId === regionRequestId.current)
        setRegionError(error instanceof Error ? error.message : 'Data wilayah gagal dimuat.');
    } finally {
  ...
  const renderRegionField = (field: RegionLevel, label: string) => {
    ...
    return (
      <div>
        ...
        <select
          required
          value={getRegionValue(field).trim()}
          disabled={Boolean(parent && !regionCodes[parent]) || regionLoading === field || options.length === 0}
          onChange={(event) => void handleRegionChange(field, event.target.value)}
          className="w-full rounded-lg border border-gray-300 bg-white p-2 text-xs disabled:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:disabled:bg-gray-800/60"
        >
          <option value="">{t('customers.selectRegion', { label })}</option>
          {options.map((option) => (
            <option key={option.code} value={regionOptionValue(option)}>
              {regionOptionValue(option)}
            </option>
          ))}
        </select>
  ...
  const handleCreateCustomerSubmit = async (e: React.FormEvent) => {
    ...
    if (hasLatitude !== hasLongitude) {
      setFormError(t('customers.invalidCoordinates'));
      return;
    }
    ...
    try {
      ...
      const address = {
        ...
        ...(latitude !== undefined && longitude !== undefined
          ...
          : {
              referenceSource: 'CUSTOMER_PROPOSED' as const,
              referencePrecision: 'UNKNOWN' as const,
  ...
  const openEditCustomer = (customer: Customer) => {
    ...
    setNewCustAddressDetail(
      [address.addressDetail, address.landmark && `Patokan: ${address.landmark}`].filter(Boolean).join(', '),
    );
      ...
      >
        ...
        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              return (
                <tr key={cust.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                  <td className="px-4 py-3.5 align-top">
                    <div className="font-mono text-[11px] text-gray-700 dark:text-gray-300 break-words whitespace-normal leading-4">
                      {cust.externalId}
                    </div>
                  ...
                  <td className="px-4 py-3.5 align-top">
                    <div className="font-semibold text-gray-900 dark:text-white break-words whitespace-normal leading-4">
                      {cust.name}
                    </div>
                    {cust.sourceRecordId && (
                      <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 break-words whitespace-normal leading-4">
                        {t('customers.sourceId')}: <span className="font-mono">{cust.sourceRecordId}</span>
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3.5 align-top">
                    {masterAddr ? (
                      <div>
                        <div className="text-gray-800 dark:text-gray-200 font-medium break-words whitespace-normal leading-4">
                          {formatAddressForDisplay(masterAddr.rawAddress)}
                        </div>
                        <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 break-words whitespace-normal leading-4">
                          {masterAddr.province} • {masterAddr.city} • {masterAddr.district} • {masterAddr.subdistrict} •{' '}
                          {masterAddr.postalCode}
                        </div>
                        {masterAddr.addressReference && (
                          <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 break-words whitespace-normal leading-4">
                            Patokan: {masterAddr.addressReference}
                          </div>
                        )}
                  ...
                  <td className="px-4 py-3.5 align-top font-mono text-[11px] text-gray-700 dark:text-gray-300 whitespace-nowrap">
                    {masterAddr?.referenceLocation
                      ? masterAddr.referenceLocation.latitude.toFixed(6)
                      : t('customers.noCoordinates')}
                  </td>

                  <td className="px-4 py-3.5 align-top font-mono text-[11px] text-gray-700 dark:text-gray-300 whitespace-nowrap">
                    {masterAddr?.referenceLocation
                      ? masterAddr.referenceLocation.longitude.toFixed(6)
                      : t('customers.noCoordinates')}
                  </td>
                  ...
                  <td className="px-4 py-3.5 align-top">
                    <span
                      className={`inline-flex items-center rounded-md border px-2 py-1 text-[10px] font-semibold ${statusBadgeClass(cust.status)}`}
                    >
                      {CUSTOMER_STATUS_LABEL[cust.status] ? t(CUSTOMER_STATUS_LABEL[cust.status]) : cust.status}
                    </span>
                    <div className="mt-1 text-[10px] text-gray-500 dark:text-gray-400 break-words whitespace-normal leading-4">
                      {t('customers.lastCheck')}:{' '}
                      {latestSession
                        ? CUSTOMER_CHECK_STATUS_LABEL[latestSession.verificationStatus]
                          ? t(CUSTOMER_CHECK_STATUS_LABEL[latestSession.verificationStatus])
                          : 'In progress'
                        : t('customers.noCheck')}
                    </div>
                    <div
                      className={`mt-1 text-[10px] font-medium break-words whitespace-normal leading-4 ${masterAddr?.isVerified || latestSession?.verificationStatus === 'LOCATION_VALID' ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-500 dark:text-gray-400'}`}
                    >
                      {t('customers.locationStatus')}:{' '}
                      {masterAddr?.isVerified || latestSession?.verificationStatus === 'LOCATION_VALID'
                        ? t('customers.locationVerified')
                        : t('customers.locationPending')}
                    </div>
                  </td>
          ...
          {!isLoading && customers.length === 0 && (
            <tr>
              <td colSpan={9} className="px-4 py-10 text-center text-xs text-gray-400 dark:text-gray-500">
                {loadError || 'Tidak ada pelanggan pada halaman ini.'}
              </td>
            </tr>
          )}
        </tbody>
