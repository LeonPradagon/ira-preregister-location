  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-3 text-xs shadow-xs dark:border-gray-800 dark:bg-gray-900 sm:flex-row">
        <div className="relative w-full sm:w-80"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Cari pelanggan, nomor HP, atau sesi..." className="w-full rounded-lg border border-gray-300 bg-white py-1.5 pl-9 pr-3 text-gray-900 outline-none focus:ring-1 focus:ring-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></div>
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="ALL">Semua status</option><option value="SCHEDULED">SCHEDULED</option><option value="PROCESSING">PROCESSING</option><option value="SENT">SENT</option><option value="FAILED">FAILED</option><option value="CANCELLED">CANCELLED</option></select>
      </div>
      ...
      >
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              {loading && <tr><td colSpan={6} className="px-4 py-10 text-center text-xs text-gray-500">Memuat pengingat...</td></tr>}
              {!loading && error && <tr><td colSpan={6} className="px-4 py-10 text-center text-xs text-rose-600">{error}</td></tr>}
              {!loading && !error && !rows.length && <tr><td colSpan={6} className="px-4 py-10 text-center text-xs text-gray-400 dark:text-gray-500">Belum ada pengingat.</td></tr>}
            </tbody>
      </AdminTable>
    </div>
  );
