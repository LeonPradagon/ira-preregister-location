...
export const VerificationListView: React.FC<VerificationListViewProps> = ({
  ...
  const { verificationSessions, customers, addresses, validationConfig } = useApp();
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('ALL');

  const filteredSessions = verificationSessions.filter((s) => {
    const cust = customers.find((c) => c.id === s.customerId);
    const matchesSearch =
      s.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      s.registeredPhoneSnapshot.includes(searchTerm) ||
      (cust && cust.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (cust && cust.externalId.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 p-3 rounded-xl border border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs shadow-xs">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <option value="ALL">Semua Status ({verificationSessions.length})</option>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right space-x-2" onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        className="px-2.5 py-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md text-[11px] font-medium inline-flex items-center gap-1 transition-colors"
