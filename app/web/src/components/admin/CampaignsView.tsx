...
export const CampaignsView: React.FC = () => {
  ...
  const loadCampaigns = async () => {
    setLoading(true);
    try {
      const response = await api.campaigns({ page, pageSize, search: searchTerm, cursor: page === 1 ? undefined : campaignCursors[page] });
  ...
  const loadCandidates = async () => {
    ...
    try {
      const response = await api.customers({ page: candidatePage, pageSize: candidatePageSize, search: customerSearch, locationStatus: 'UNVERIFIED', cursor: candidatePage === 1 ? undefined : candidateCursors[candidatePage] });
      if (response.nextCursor) setCandidateCursors((previous) => ({ ...previous, [candidatePage + 1]: response.nextCursor! }));
  ...
  const showItems = async (campaignId: string, nextPage = 1, nextPageSize = itemPageSize) => {
    ...
    try {
      const response = await api.campaignItems(campaignId, { page: nextPage, pageSize: nextPageSize, cursor: nextPage === 1 ? undefined : itemCursors[nextPage] });
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-xs dark:border-gray-800 dark:bg-gray-900">
      ...
      <form onSubmit={(event) => void submit(event)} className="mt-5 space-y-4">
        ...
        <TablePagination page={candidatePage} pageSize={candidatePageSize} total={candidateTotal} onPageChange={setCandidatePage} onPageSizeChange={(size) => { setCandidatePageSize(size); setCandidatePage(1); setCandidateCursors({}); }} disabled={candidateLoading} />
