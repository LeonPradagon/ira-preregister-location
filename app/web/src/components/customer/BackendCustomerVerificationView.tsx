...
const ValidationEvidence: React.FC<{ decision: ServerValidationDecision }> = ({ decision }) => <div className="rounded-lg border border-amber-200 bg-white/70 p-3 text-[11px] text-slate-700"><div className="grid grid-cols-2 gap-2"><span>Akurasi GPS: <strong>±{decision.bestSample.accuracyMeters.toFixed(1)} m</strong></span><span>Spread sampel: <strong>{decision.sampleSpreadMeters.toFixed(1)} m</strong></span><span>Jarak ke referensi: <strong>{decision.distanceFromReferenceMeters == null ? 'belum tersedia' : `${decision.distanceFromReferenceMeters.toFixed(1)} m`}</strong></span><span>Skor alamat: <strong>{Math.round(decision.addressScore * 100)}%</strong></span></div><p className="mt-2 break-words text-slate-500">Alasan: {decision.reasonCodes.join(', ')}</p></div>;
