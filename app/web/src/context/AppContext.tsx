...
type NewCustomerAddress = Omit<
  CustomerAddress,
  'id' | 'customerId' | 'createdAt' | 'updatedAt' | 'isActive' | 'isVerified' | 'referenceLocation'
> & {
...
const DEFAULT_INTEGRATION_CONFIGS: IntegrationConfigs = {
  IRA_COVERAGE: {
    enabled: false,
    name: 'IRA Coverage',
    description: 'Coverage verification integration',
    status: 'PENDING_EXTERNAL_CONTRACT',
  },
  TICKETING: {
    enabled: false,
    name: 'Ticketing',
    description: 'Dispatch ticket integration',
    status: 'PENDING_EXTERNAL_CONTRACT',
  },
};
...
interface AppContextType {
  ...
  addCustomer: (
    ...
    addressData: NewCustomerAddress,
  ) => Promise<Customer>;
...
function mapApiDashboard(raw: AdminDashboardApi): DashboardSummary {
  ...
  const numberMap = (value: Record<string, unknown> | undefined) =>
    Object.fromEntries(Object.entries(value ?? {}).map(([key, item]) => [key, number(item)]));
  return {
    generatedAt: String(raw.generatedAt ?? ''),
    countAsOf: raw.countAsOf ? String(raw.countAsOf) : undefined,
    customers: {
      total: number(raw.customers.total),
      active: number(raw.customers.active),
      verified: number(raw.customers.verified),
      whatsappOptedIn: number(raw.customers.whatsappOptedIn),
      whatsappOptedOut: number(raw.customers.whatsappOptedOut),
    },
    verifications: {
      total: number(raw.verifications.total),
      invitationsSent: number(raw.verifications.invitationsSent),
      linksOpened: number(raw.verifications.linksOpened),
      customersConfirmed: number(raw.verifications.customersConfirmed),
      customersMismatch: number(raw.verifications.customersMismatch),
      gpsCaptured: number(raw.verifications.gpsCaptured),
      lowGpsAccuracy: number(raw.verifications.lowGpsAccuracy),
      waitingForHome: number(raw.verifications.waitingForHome),
      addressChanged: number(raw.verifications.addressChanged),
    ...
    reminders: {
      ...
      scheduled: number(raw.reminders.scheduled),
    ...
    outbox: {
      total: number(raw.outbox.total),
      pending: number(raw.outbox.pending),
      published: number(raw.outbox.published),
      failed: number(raw.outbox.failed),
    },
  };
}
...
export function mapApiCustomer(raw: Record<string, unknown>): Customer {
  return {
    ...
    status: String(raw.status ?? 'ACTIVE') as Customer['status'],
    sourceRecordId: raw.sourceRecordId ? String(raw.sourceRecordId) : undefined,
    sourceCreatedAt: raw.sourceCreatedAt ? String(raw.sourceCreatedAt) : undefined,
    ...
    sourceMetadata:
      raw.sourceMetadata && typeof raw.sourceMetadata === 'object'
        ...
        : undefined,
    activeAddress:
      raw.activeAddress && typeof raw.activeAddress === 'object'
        ? mapApiAddress(raw.activeAddress as Record<string, unknown>)
        : null,
    latestVerification:
      raw.latestVerification && typeof raw.latestVerification === 'object'
        ? mapApiSession(raw.latestVerification as Record<string, unknown>)
        : null,
...
function mapApiAddress(raw: Record<string, unknown>): CustomerAddress {
  return {
    ...
    referenceConfidence: Number(raw.referenceConfidence ?? 0),
    referenceLocation: mapApiReferenceLocation(raw.referenceLocation),
  };
}

export function mapApiSession(raw: Record<string, unknown>): VerificationSession {
  return {
    ...(raw as unknown as VerificationSession),
    id: String(raw.id),
    customerId: String(raw.customerId),
    ...
    lastValidationResult:
      raw.lastValidationResult && typeof raw.lastValidationResult === 'object'
        ? mapApiValidationResult(raw.lastValidationResult as Record<string, unknown>)
        : undefined,
  };
}

export function mapApiValidationResult(raw: Record<string, unknown>): ValidationResult {
  const latitude = Number(raw.capturedLatitude);
  const longitude = Number(raw.capturedLongitude);
  const accuracyMeters = Number(raw.gpsAccuracyMeters);
  const precision = String(raw.referencePrecision ?? 'UNKNOWN') as ValidationResult['referencePrecision'];
  return {
    ...(raw as unknown as ValidationResult),
    id: String(raw.id),
    sessionId: String(raw.sessionId),
    captureId: String(raw.captureId),
    addressId: String(raw.addressId),
    gpsAccuracyM: accuracyMeters,
    distanceToReferenceM: raw.distanceToReferenceMeters == null ? null : Number(raw.distanceToReferenceMeters),
    distanceFromReferenceMeters: raw.distanceToReferenceMeters == null ? null : Number(raw.distanceToReferenceMeters),
    capturedLocation: {
      latitude,
      longitude,
      accuracyMeters,
      capturedAt: String(raw.createdAt),
      coordinateText: `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`,
      googleMapsUrl: `https://www.google.com/maps/search/?api=1&query=${latitude},${longitude}`,
    },
    referenceLocation:
      raw.referenceLatitude == null || raw.referenceLongitude == null
        ? null
        : { latitude: Number(raw.referenceLatitude), longitude: Number(raw.referenceLongitude), precision },
...
function mapApiCapture(raw: Record<string, unknown>): LocationCapture {
  return {
    ...(raw as unknown as LocationCapture),
    id: String(raw.id),
    sessionId: String(raw.sessionId),
    latitude: Number(raw.latitude),
    longitude: Number(raw.longitude),
    accuracyMeters: Number(raw.accuracyMeters),
    bestAccuracyMeters: Number(raw.bestAccuracyMeters),
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [theme, setThemeState] = useState<ThemeMode>(() => {
    const saved = localStorage.getItem('el_theme') as ThemeMode | null;
  ...
  const getCustomerSessions = (customerId: string) =>
    verificationSessions.filter((session) => session.customerId === customerId);
  ...
  const loadCustomerDetail = async (customerId: string) => {
    ...
    const detailSessions = (Array.isArray(raw.sessions) ? raw.sessions : []).map((row) =>
      mapApiSession(row as Record<string, unknown>),
    );
    setCustomers((prev) =>
      prev.some((item) => item.id === customerId)
        ? prev.map((item) => (item.id === customerId ? customer : item))
        : [customer, ...prev],
    );
    setCustomerPage((prev) => ({
      ...prev,
      items: prev.items.map((item) => (item.id === customerId ? customer : item)),
    }));
  ...
  const loadVerificationDetail = async (sessionId: string) => {
    ...
    if (Array.isArray(raw.captures))
      setLocationCaptures((previous) => [
        ...(raw.captures as Record<string, unknown>[]).map(mapApiCapture),
        ...previous.filter((item) => item.sessionId !== session.id),
      ]);
    if (Array.isArray(raw.reviews))
      setVerificationReviews((previous) => [
        ...(raw.reviews as VerificationReview[]),
        ...previous.filter((item) => item.sessionId !== session.id),
      ]);
  ...
  const addCustomer = async (
    ...
    const payload = await api.createCustomer({ ...customerData, address: addressData });
    const mappedAddress = mapApiAddress(payload.address as Record<string, unknown>);
    const customer = { ...mapApiCustomer(payload.customer as Record<string, unknown>), activeAddress: mappedAddress };
    setCustomers((prev) => [customer, ...prev]);
    setCustomerPage((prev) => {
      const total = prev.total + 1;
      return {
        ...prev,
        items: [customer, ...prev.items].slice(0, prev.pageSize),
        total,
  ...
  const deleteCustomer = async (customerId: string): Promise<void> => {
    ...
    const deletedSessionIds = new Set(
      verificationSessions.filter((item) => item.customerId === customerId).map((item) => item.id),
    );
  ...
  const createVerificationSession = async (customerId: string, addressId: string): Promise<VerificationSession> => {
    ...
    if (!customer || !address) throw new Error('Customer or address not found');
    const result = await api.createVerification(customerId, addressId);
    ...
    setVerificationSessions((prev) => [session, ...prev.filter((item) => item.id !== session.id)]);
    await refreshDashboard();
    return session;
  };
  const resendInvitation = async (sessionId: string) => {
    assertCapability(currentAdmin?.role, 'sendVerification');
    const result = await api.resend(sessionId);
    setVerificationSessions((prev) =>
      prev.map((session) =>
        session.id === sessionId
          ? { ...session, expiresAt: result.expiresAt, updatedAt: new Date().toISOString() }
          : session,
      ),
    );
    await refreshDashboard();
  ...
  const sendManualReminder = async (sessionId: string) => {
    try {
      assertCapability(currentAdmin?.role, 'sendVerification');
      const result = await api.reminder(sessionId);
      setVerificationSessions((prev) =>
        prev.map((session) =>
          session.id === sessionId
            ? {
                ...session,
                reminderCount: result.reminderNumber,
                verificationStatus: result.status as VerificationSession['verificationStatus'],
      ...
      return { success: false, message: error instanceof Error ? error.message : 'Pengingat gagal dijadwalkan.' };
    }
  };
  const createCampaign = async (
    ...
    assertCapability(currentAdmin?.role, 'createVerification');
    const raw = await api.createCampaign({
      name,
      ...(options?.targetFilter ? { targetFilter: options.targetFilter } : { customerIds }),
      batchSize: options?.batchSize ?? 500,
      dailySendLimit: options?.dailySendLimit ?? 500,
      sendWindowDays: options?.sendWindowDays ?? 0,
      scheduledAt,
    ...
    const campaign = {
      ...
      id: String(raw.id),
      name: String(raw.name),
      status: String(raw.status) as VerificationCampaign['status'],
  ...
  const optOutCustomer = async (customerId: string) => {
    ...
    setCustomers((prev) =>
      prev.map((customer) =>
        customer.id === customerId ? { ...customer, whatsappOptOutAt: new Date().toISOString() } : customer,
      ),
    );
    await refreshDashboard();
  };
  const performManualReview = async (
    sessionId: string,
    decision: ReviewDecision,
    reasonCode: string,
    reviewNote: string,
    ...
    const result = await api.review(sessionId, { decision, reasonCode, reviewNote });
    await loadVerificationDetail(sessionId);
    await refreshDashboard();
    if (result.status !== 'LOCATION_VALID')
      setVerificationSessions((prev) =>
        prev.map((session) =>
          session.id === sessionId
            ? {
  ...
  const updateAddressFromGps = async (sessionId: string) => {
    assertCapability(currentAdmin?.role, 'manualReview');
    const result = await api.addressFromGps(sessionId);
    await loadVerificationDetail(sessionId);
    await refreshDashboard();
    return result.updatedFields;
  };
  const updateValidationConfig = async (newConfig: Partial<ValidationConfig>) => {
